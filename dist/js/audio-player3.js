function CtPlayer(el) {
    ...
    this.closeVolumeMouseleave = false

    this.initPlayer = function () {
        this.container.addEventListener('mousedown', this.mouseDown.bind(this))

        if(this.closeVolumeMouseleave)
        ...
        this.wavesurfer = WaveSurfer.create({
            ...
            cursorWidth: 2,
            hideScrollbar: true,
            backend: 'MediaElement', //HTML5 audio tag, instead 'WebAudio'
        })
    ...
    this.directionAware = function () {
        //без перемещения вбок
        // if(window.innerHeight < 250) {
        //     this.volumeControls.style.bottom = '-54px'
        //     this.volumeControls.style.left = '54px'
        // } else if(el.offsetTop < 154) {  //кол-во пикселей отступа сверху относительно родителя
        //     this.volumeControls.style.bottom = '-164px'
        //     this.volumeControls.style.left = '-3px'
        // } else { //где-то разве надо? зачем вообще, если по умолчанию такие же зн-я
